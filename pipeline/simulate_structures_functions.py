...
from config_pipeline import * #Parameters and paths
...
def create_files(pdbfile, smalmol, name, smalmol_folder, hydrogenate_ligand=True):
    ...
    if os.path.exists(smalmol_mol2):
        print("Ligand %s already has mol2 file. Skipping"%(smalmol))
        return
        pass
...
def define_equilibration(builddir, outdir, parameters, const_sel = 'protein and name C CA N O or not (protein or lipid or water or ions) and noh or segid IO WAT and noh', 
...
def get_uniprot_alignment(mymol, segpos, uniprotkbac, isoform, entry_segs):
    ...
    for seg,myseq in mymol.sequence().items():
        ...
        aligs = aligner.align(myseq, uniseq)
        # For every alignment
        count = 1
        for alig in aligs: 
            if count <= 10: #some alignments are like 807218970624 and goes to infinity, only the first 10
                a = alig.coordinates # List of tuples with start-end of matches in both sequences
                start = a[0][0] # Start position of the first alignment in the query sequence
                end = a[0][-1] # End position of the last alignment in the query sequence
                # If the alignment has no significant amount of mismatches, keep it as segment
                if (alig.score/(end-start)) > 3.5:
                    
                    starting = start
                    ending = a[0][-1] - 1 # -1 because the end position is from python
